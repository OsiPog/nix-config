...
in {
  network.sharedModules = [
    ({
      ...
      config = {
        assertions =
          flatten (mapAttrsToList (portName: portCfg: [
            {
              assertion = (portCfg.port == null) != (portCfg.portRange == null);
              message = "Port \"${portName}\" on \"${name}\" must have exactly one of 'port' or 'portRange' defined (not both, not neither).";
            }
            # In port range from should be less than or equal to to
            {
              ...
              message = "Port \"${portName}\" on \"${name}\" has invalid port range: 'from' (${toString portCfg.portRange.from}) must be <= 'to' (${toString portCfg.portRange.to}).";
            }
            # Check that virtual-host reverse proxy has a subdomain
            {
              ...
              message = "Port \"${portName}\" on \"${name}\" uses virtual-host reverse proxy but no domain is specified.";
            }
            # Check that port ranges don't use virtual-host method
            {
              ...
              message = "Port \"${portName}\" on \"${name}\" has a port range and cannot use 'virtual-host' reverse proxy method. Use 'stream' instead.";
            }
            # Check that reverse proxy host is actually a reverse proxy
            {
              ...
              message = "No domain of any host is suffix of the reverse proxy domain \"${portCfg.reverseProxy.domain}\" of port \"${portName}\" on \"${name}\".";
            }
            {
