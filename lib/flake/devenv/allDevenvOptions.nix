flake: let
    ...
    pipe set [
      ...
      (map (
        name:
          "${name}="
          + (
            if (typeOf set.${name} == "set")
            then stringSetToString set.${name}
            else "\"${set.${name}}\""
          )
          + ";"
      ))
  ...
  projectOptions = pipe ((getEnv "PWD") + "/.devenv.flake.nix") [
...
in
  ...
  (
    if hasAttr "devenv" flake.inputs
    then devenvOptions
    else {}
  )
  //
  ...
  (
    if pathExists ((getEnv "PWD") + "/.devenv.flake.nix")
    then projectOptions
    else {}
  )
